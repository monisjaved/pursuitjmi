...
if (isset($_POST['pass']))
{
	...
	if($pass!="" || $pass !=NULL ) // password is not blank 
		{	
			$verify=0;
			if($pass=="tangeloTowniswatingforyou!!") 
				$flag =1;
			else
			{
...
if($flag==1)
{
	  	$query="UPDATE `level` SET `level`=11,`time`=".time()." WHERE uid = ".$user."";
        	$q=mysql_query($query);
        	if(!q)
...
if($flag==0){
?></strong>Enter Password To Move to Next Level :</strong><form name="input" action="" method="post">
<strong>Password: </strong><input type="text" name="pass">
<input type="submit" value="Submit">
</form><br/><center><img src="level/pic.jpg" height="200px" weight="<20"><br/><br/><hr/>
</center><?php } 
