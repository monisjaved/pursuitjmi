...
if (isset($_POST['pass']))
{
	...
	if($pass!="" || $pass !=NULL ) // password is not blank 
		{	
			$verify=0;
			if($pass=="stillyouarelookingforpassword") 
				$flag =1;
			else
			{
...
if($flag==1)
{
	  	$query="UPDATE `level` SET `level`=14,`time`=".time()." WHERE uid = ".$user."";
        	$q=mysql_query($query);
        	if(!q)
...
if($flag==0){
?><center><img src="level/pic2.png"><form name="input" action="" method="post">
